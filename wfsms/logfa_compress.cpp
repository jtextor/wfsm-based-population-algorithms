...
static void report(const logfa &fa) {
...
int main(int argc, char **argv) {
    ...
    std::vector<logfa> automata;
    std::string s(length, '0');
    std::string t(length, '2');
    std::cout << "nstrings,nstates,narcs\n";
    for (;;) {
        automata.push_back(singleton<logfa>(s));
    ...
    std::vector<logfa> folded;
    std::vector<logfa> &a = automata, &b = folded;
    while (1 < a.size()) {
